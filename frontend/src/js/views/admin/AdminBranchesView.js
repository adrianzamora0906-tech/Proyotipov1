...
export default class AdminBranchesView extends Component {
  ...
  async openCourseProgramForm(body, course, page = false) {
    ...
    const weekendOverrideRow=(item={})=>{const selected=item.instructors||[],count=Math.min(Math.max(Number(item.instructor_count||2),1),3);return `<div class="weekend-capacity-row">
      ...
      <label class="weekend-third-instructor"><span>Tercer instructor</span><select class="weekend-instructor-3">${weekendInstructorOptions(selected[2]?.id)}</select></label>
    ...
    const modal=page?body:document.getElementById('branch-modal');modal.innerHTML=`${page?'<div class="course-program-page__top"><button type="button" class="btn btn-light close-modal">← Volver a cursos</button></div>':'<div class="branch-modal-backdrop">'}<form class="${page?'course-program-page__form':'branch-modal course-program-modal'}" id="course-program-form" novalidate>
      ...
      <div class="course-program-body">
        ...
        <section class="course-program-section weekend-capacity-section"><div class="course-program-section__title"><div><strong>Capacidad especial por fin de semana</strong><small>Publica uno, dos o tres instructores desde el inicio para una fecha concreta. Cada instructor configurado consume su turno en la rotación.</small></div><button type="button" class="btn btn-light" id="add-weekend-capacity">+ Configurar fecha</button></div><div id="weekend-capacity-list">${weekendOverrides.map(weekendOverrideRow).join('')}</div><p class="branch-empty weekend-capacity-empty" ${weekendOverrides.length?'hidden':''}>No hay excepciones futuras. Se aplicará la rotación normal de un instructor.</p></section>
