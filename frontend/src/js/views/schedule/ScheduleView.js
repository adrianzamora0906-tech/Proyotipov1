...
class ScheduleView extends Component {
  ...
  renderMonthlyAvailability(calendar) {
    ...
    const statusCell=(slot,date)=>{const value=slot.occupancyByDate?.[date],available=Number(value?.available)||0;if(!value||available<=0)return{state:'unavailable',visible:false,label:''};return{state:'free',visible:true,label:`${available} cupo${available===1?'':'s'} disponible${available===1?'':'s'}`};};
    ...
    const table=visibleDates.length&&availableSlots.length?`<div class="rolling-week-table cycle-availability-table" style="--visible-course-days:${visibleDates.length}"><div class="day-table-heading">Hora</div>${visibleDates.map(date=>`<div class="day-table-heading">${formatDate(date)}</div>`).join('')}${availableSlots.map(slot=>`<div class="day-table-time">${slot.startTime} – ${slot.endTime}</div>${visibleDates.map(date=>{const status=statusCell(slot,date);return`<div class="day-table-cell ${status.state}">${status.visible?`<span class="slot-status rolling-slot-available">Disponible</span><small class="rolling-slot-count">${status.label}</small>`:''}</div>`;}).join('')}`).join('')}</div>`:'<div class="schedule-empty">Este curso no tiene horarios completamente disponibles en sus días configurados.</div>';
