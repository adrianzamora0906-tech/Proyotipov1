    ...
    return `<div class="student-portal">
      ...
      <main class="student-portal__main">
        <section data-student-panel="summary">
          ...
          <article class="student-card"><div class="student-card__title"><h2>Estado de pago</h2><strong>${money(payment?.balance)} pendiente</strong></div><dl><div><dt>Valor del curso</dt><dd>${money(payment?.final_amount)}</dd></div><div><dt>Pagado</dt><dd>${money(payment?.paid)}</dd></div></dl><div class="student-payment ${Number(payment?.balance)>0?'pending':'paid'}">${Number(payment?.balance)>0?'Tienes un saldo pendiente':'Curso pagado completamente'}</div></article></div>
        </section>
