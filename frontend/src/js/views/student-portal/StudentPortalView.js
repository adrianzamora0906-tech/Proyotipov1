...
const time=value=>String(value||'').slice(0,5)||'Por asignar';
...
export default class StudentPortalView extends Component{
  ...
  async render(){
    ...
    const theoryStart=hasVirtualTheory?'Disponible en el aula virtual':date(enrollment?.theory_group_start_date);
    const theorySchedule=hasVirtualTheory
      ? 'Modalidad virtual'
      : enrollment?.theory_group_start_time
        ? `${time(enrollment.theory_group_start_time)} – ${time(enrollment.theory_group_end_time)}`
        : 'Por asignar';
    return `<div class="student-portal">
      ...
      <main class="student-portal__main">
        <section data-student-panel="summary">
          ...
          <div class="student-portal__grid"><article class="student-card"><div class="student-card__title"><h2>Mi curso</h2><span class="student-status">${esc(enrollment?.cycle_status||student.status)}</span></div><h3>${esc(enrollment?.course_name||'Sin curso asignado')}</h3><dl><div><dt>Instructor práctico</dt><dd>${esc(enrollment?.instructor_name||'Por asignar')}</dd></div><div><dt>Inicio de prácticas</dt><dd>${date(enrollment?.start_date)}</dd></div><div><dt>Finalización</dt><dd>${date(enrollment?.end_date)}</dd></div><div><dt>Modalidad teórica</dt><dd>${esc((enrollment?.theory_modality||'presencial_regular').replaceAll('_',' '))}</dd></div><div><dt>Inicio de clases teóricas</dt><dd>${esc(theoryStart)}</dd></div><div><dt>Horario teórico</dt><dd>${esc(theorySchedule)}</dd></div>${!hasVirtualTheory?`<div><dt>Profesor de teoría</dt><dd>${esc(enrollment?.theory_instructor_name||'Por asignar')}</dd></div>`:''}</dl><div class="student-progress"><span style="width:${progress}%"></span></div><small>${progress}% de clases registradas como asistidas</small></article>
