...
class StudentsView extends Component {
            ...
            <div class="modal-body student-modal-body">
              ...
              <section class="student-modal-step" data-step="2">
                <fieldset class="student-registration-types">
                  ...
                  <label class="student-registration-type is-selected"><input type="radio" name="registrationMode" value="regular" checked><span class="student-registration-type__check"></span><span><strong>Curso de conducción</strong></span></label>
                  <label class="student-registration-type"><input type="radio" name="registrationMode" value="additional-practice"><span class="student-registration-type__check"></span><span><strong>Horas prácticas</strong></span></label>
                  <label class="student-registration-type"><input type="radio" name="registrationMode" value="license-renewal"><span class="student-registration-type__check"></span><span><strong>Examen para renovación</strong></span></label>
              ...
              <section class="student-modal-step active" data-step="1">
                ...
                <div class="external-instructor-selector" id="external-instructor-selector" hidden></div>
                ...
                <div class="schedule-workflow-heading schedule-workflow-heading--calendar">
                  ...
                  <div><h4 id="practice-schedule-title">Fecha y horario</h4></div>
                </div>
                ...
                <div class="schedule-workflow-heading">
                  ...
                  <div><h4 id="theory-schedule-title">Horario de teor&iacute;a</h4></div>
                </div>
  ...
  async mountStudentModalEvents() {
    ...
    document.getElementById('student-schedule-calendar')?.addEventListener('click', async event => {
      ...
      if (preferredSelect && instructorId && [...preferredSelect.options].some(option => option.value === instructorId)) {
        ...
        option.textContent = instructorButton.dataset.courseInstructorName || instructorButton.textContent.trim();
    ...
    form?.querySelector('[name="course_id"]')?.addEventListener('change', async () => {
      ...
      if (branchId) {
        ...
        await this.reloadModalSchedules(branchId);
      } else {
    ...
    document.getElementById('external-instructor-selector')?.addEventListener('click', event => {
      const button = event.target.closest('[data-external-instructor-id]');
      if (!button) return;
      const select = document.getElementById('preferred-instructor-select');
      const instructorId = button.dataset.externalInstructorId || '';
      if (select && instructorId && ![...select.options].some(option => option.value === instructorId)) {
        select.add(new Option(button.textContent.trim(), instructorId));
      }
      if (select) {
  ...
  renderEnrollmentCycleCalendar({ courseKey, modality, title, schedules, cycle, cycleIndex, cycleCount, activeCycleIndex }) {
    ...
    const displayStartDate = String(cycle.officialStartDate || cycle.startDate) < String(cycle.startDate)
      ? cycle.officialStartDate : cycle.startDate;
    const displayEndDate = String(cycle.officialEndDate || cycle.endDate) > String(cycle.endDate)
      ? cycle.officialEndDate : cycle.endDate;
    const days = this.getCourseBusinessDays(displayStartDate, displayEndDate, courseKey, modality);
    const selectedExternalInstructor = (this.modalBranchInstructors || []).some(instructor =>
      String(instructor.id) === String(this.scheduleInstructorFilterId || '')
      && instructor.branch_assignment === 'reserved_elsewhere');
    ...
    return `
      <div class="enrollment-calendar" data-course="${courseKey}" data-modality="${modality}" data-cycle-id="${escapeHtml(cycle.key)}" data-cycle-index="${cycleIndex}" data-day-window-start="0" data-day-count="${days.length}" data-required-classes="${Number(cycle.durationBusinessDays) || days.length}" style="display: ${cycleIndex === activeCycleIndex ? 'block' : 'none'};">
        ...
        ${cycle.instructors.length && !selectedExternalInstructor ? `
  ...
  async getSchedulesForModal(branchId = null, instructorId = null, practicalStartDate = null, practicalCycleId = null) {
    try {
      const branchFilter = {
  ...
  async reloadModalSchedules(branchId, instructorId = null, practicalStartDate = null, options = {}) {
    ...
    const activeCycleIds = new Map([...host.querySelectorAll('.enrollment-cycle-set')].map(set => {
      const activeIndex = Number(set.dataset.activeCycleIndex || 0);
      const activeCalendar = [...set.querySelectorAll(':scope > .enrollment-calendar')]
        .find(calendar => Number(calendar.dataset.cycleIndex || 0) === activeIndex);
      return [`${set.dataset.course || ''}|${set.dataset.modality || ''}`, activeCalendar?.dataset.cycleId || ''];
    }));
    ...
    const allSchedulesKey = `${branchId}|${selectedPracticalStart || ''}|${selectedPracticalCycleId || ''}`;
    ...
    if (shouldFilterByInstructor && selectedCourse) {
      ...
      schedules = await this.getSchedulesForModal(branchId, null, selectedPracticalStart || null, selectedPracticalCycleId);
    ...
    if (selectedInstructorId && shouldFilterByInstructor) {
      allSchedules ||= !forceAllSchedulesRefresh && this.modalAllSchedulesKey === allSchedulesKey
        ...
        : null;
      allSchedules ||= await this.getSchedulesForModal(branchId, null, selectedPracticalStart || null, selectedPracticalCycleId);
    ...
    const nextCalendarByKey = new Map([...nextCalendar.querySelectorAll('.enrollment-calendar')]
      .map(calendar => [`${calendar.dataset.course || ''}|${calendar.dataset.modality || ''}|${calendar.dataset.cycleIndex || ''}`, calendar]));
    ...
    const canUpdateOnlyGrids = currentGrids.length > 0
      ...
      && currentGrids.every(grid => {
        const key = gridKey(grid);
        const currentCalendar = grid.closest('.enrollment-calendar');
        const updatedCalendar = nextCalendarByKey.get(key);
        return nextGridByKey.has(key)
          && updatedCalendar
          && currentCalendar?.dataset.dayCount === updatedCalendar.dataset.dayCount;
      });
  ...
  closeStudentModal() {
    ...
    document.querySelectorAll('.mobile-registration-capture-overlay').forEach(overlay => overlay.remove());
    clearTimeout(this.additionalPracticeAvailabilityTimer);
    clearTimeout(this.additionalPracticeLookupTimer);
    ...
    this.instructorFirstAvailabilityDate = null;
    this.showCantonReferralInstructors = false;
    this.modalAllSchedules = null;
    this.modalAllSchedulesKey = null;
    this.additionalPracticeStudent = null;
    this.additionalPracticeResolved = false;
    this.resolvedAdditionalPracticeIdentification = '';
    const modality = document.getElementById('selected-enrollment-modality');
    if (modality) modality.value = 'normal';
    document.querySelectorAll('#student-schedule-calendar .enrollment-modality-button[data-modality]').forEach(item => {
      const selected = item.dataset.modality === 'normal';
      item.classList.toggle('active', selected);
      item.setAttribute('aria-pressed', String(selected));
    });
    ...
    document.querySelectorAll('.student-registration-type').forEach(card => {
      card.classList.toggle('is-selected', card.querySelector('input')?.value === 'regular');
    });
    document.querySelectorAll('.renewal-hidden').forEach(element => { element.hidden = false; });
    document.querySelectorAll('.renewal-observations-field').forEach(element => { element.hidden = true; });
    document.querySelector('.student-modal-steps')?.classList.remove('renewal-mode');
    const advancedFields = document.getElementById('advanced-practical-start-fields');
    if (advancedFields) advancedFields.hidden = true;
    const paymentFields = document.getElementById('student-payment-fields');
    if (paymentFields) paymentFields.style.display = 'none';
    const mobileCaptureButton = document.getElementById('mobile-registration-package-capture');
    if (mobileCaptureButton) mobileCaptureButton.textContent = 'Capturar documentos desde teléfono';
    const additionalCaptureButton = document.getElementById('mobile-additional-practice-capture');
    if (additionalCaptureButton) additionalCaptureButton.textContent = 'Capturar desde teléfono';
    form?.querySelectorAll('.registration-document-saved-status').forEach(status => { status.textContent = ''; });
    form?.querySelectorAll('[data-file-name]').forEach(label => { label.textContent = 'Ningún archivo seleccionado'; });
    this.updateScheduleSelectionSummary([]);
  ...
  async handleScheduleCellClick(option) {
    ...
    if (!rotationEnabled) {
      calendar.querySelectorAll('.schedule-option').forEach(cell => {
        cell.classList.toggle('selected', cell.dataset.time === option.dataset.time
          && cell.dataset.date >= option.dataset.date);
      });
      this.storeSchedulePlan(calendar);
      await this.extendFixedScheduleCalendar(calendar);
      return;
    }
  ...
  async extendFixedScheduleCalendar(calendar) {
    const schedulePlan = this.parseSchedulePlan(document.getElementById('selected-schedule-plan')?.value);
    if (!calendar || schedulePlan.rotation || !schedulePlan.selections.length) return;
    const visibleDates = [...calendar.querySelectorAll('.schedule-option[data-date]')]
      .map(option => option.dataset.date).filter(Boolean).sort();
    const selectedDates = schedulePlan.selections.map(selection => selection.date).filter(Boolean).sort();
    if (!visibleDates.length || !selectedDates.length
      || selectedDates[selectedDates.length - 1] <= visibleDates[visibleDates.length - 1]) return;
    ...
    const instructorId = this.scheduleInstructorFilterId
      ...
      || null;
    const cycleId = schedulePlan.selections[0].cycleId;
    const selectedKeys = new Set(schedulePlan.selections.map(selection => `${selection.date}|${selection.time}`));
    await this.reloadModalSchedules(branchId, instructorId, selectedDates[0], { practicalCycleId: cycleId });
    const refreshed = [...document.querySelectorAll(`.enrollment-calendar[data-cycle-id="${cycleId}"]`)]
      .find(item => item.style.display !== 'none')
      || document.querySelector(`.enrollment-calendar[data-cycle-id="${cycleId}"]`);
    if (!refreshed) return;
    refreshed.querySelectorAll('.schedule-option').forEach(cell => {
      cell.classList.toggle('selected', selectedKeys.has(`${cell.dataset.date}|${cell.dataset.time}`));
    });
  ...
  storeSchedulePlan(calendar) {
    ...
    if (calendar && plan.length && !examOnly && !this.isRotationEnabled(calendar)) {
      ...
      plan = dates.map((date, index) => ({
        ...first,
        // Conserva el identificador cycle:* para que el guardado utilice la
        // reserva transaccional de ciclos y no el flujo legado de horarios.
        id: first.id,
        date,
        dayIndex: index,
        practicalStartDate: dates[0],
      }));
    }
  ...
  async handleStudentSubmit(event) {
    ...
    const selectedCellsAreAvailable = examOnly ? schedulePlan.selections.length === 1 : this.activatingReservation
      ...
      && schedulePlan.selections.every(selection => {
        ...
        const visibleDates = Object.keys(schedule?.availabilityByDate || {}).sort();
        const continuesAfterVisibleCourse = Boolean(schedule && visibleDates.length
          && selection.date > visibleDates[visibleDates.length - 1]);
        return schedule && (continuesAfterVisibleCourse
          || (dateAvailability && Number(dateAvailability.available) > 0));
      });
    ...
    try {
      ...
      // ================================================================
      // REGLA PROTEGIDA: CIERRE OBLIGATORIO DEL MODAL DE NUEVO ESTUDIANTE
      // No mover, eliminar ni condicionar este cierre. Cuando el servidor
      // confirma el horario, el modal debe cerrarse antes de ejecutar pagos,
      // notificaciones o cualquier otra tarea secundaria.
  ...
  async selectStudentSchedule(studentId, scheduleId, schedulePlan = null) {
    const hasCycleSelection = Array.isArray(schedulePlan?.selections)
      && schedulePlan.selections.some(selection => Boolean(selection?.cycleId));
    if (String(scheduleId).startsWith('cycle:') || hasCycleSelection) {
